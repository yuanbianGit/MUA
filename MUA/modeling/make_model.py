...
from modeling.backbones.vit_pytorch import vit_base_patch16_224, vit_small_patch16_224, \
    deit_small_patch16_224,vit_base_att
...
class build_transformer_att(nn.Module):
    def __init__(self, num_classes, cfg, camera_num, view_num, factory):
        ...
        self.base = factory["vit_att"](img_size=cfg.INPUT.SIZE_TRAIN, sie_xishu=cfg.MODEL.SIE_COE,
                                                        num_classes=num_classes,
                                                        camera=camera_num, view=view_num,
                                                        stride_size=cfg.MODEL.STRIDE_SIZE,
                                                        drop_path_rate=cfg.MODEL.DROP_PATH,
    ...
    def forward(self, x, layer=1, label=None, cam_label=None, view_label=None):
        # inter_map (b,129,768)
        cash_x, inter_map = self.base(x, cam_label=cam_label, view_label=view_label, layer=layer)
        global_feat = cash_x[-1][:, 0]
        inter_map = inter_map[:,1:,:]
        inter_map = einops.rearrange(inter_map, 'b (h w) l -> b l h w', h=16)
        ...
        if self.training:
            ...
            if self.neck_feat == 'after':
                return cash_x, feat,inter_map
            else:
                return cash_x, global_feat,inter_map
...
class BASELINE(nn.Module):
    ...
    def forward(self, x, label=None, cam_label=None, view_label=None):
        if self.training:
            ...
            NI = x['NI']
            TI = x['TI']
            NI_cash, NI_global = self.NI(NI, layer=self.layer,cam_label=cam_label, view_label=view_label)
            TI_cash, TI_global = self.TI(TI, layer=self.layer,cam_label=cam_label, view_label=view_label)
            RGB_cash, RGB_global = self.RGB(RGB, layer=self.layer,cam_label=cam_label, view_label=view_label)
            ori = torch.cat([RGB_global, NI_global, TI_global], dim=-1)
            ori_global = self.bottleneck(ori)
...
class TOPReID_att(nn.Module):
    def __init__(self, num_classes, cfg, camera_num, view_num, factory):
        super(TOPReID_att, self).__init__()
        self.NI = build_transformer_att(num_classes, cfg, camera_num, view_num, factory)
        self.TI = build_transformer_att(num_classes, cfg, camera_num, view_num, factory)
        self.RGB = build_transformer_att(num_classes, cfg, camera_num, view_num, factory)
        self.feat_layer = cfg.SOLVER.LAYER
    ...
    def forward(self, x, cam_label=None, label=None, view_label=None):
        ...
        NI_cash, NI_global,NI_map = self.NI(NI,layer=self.feat_layer, cam_label=cam_label, view_label=view_label)
        TI_cash, TI_global,TI_map = self.TI(TI, layer=self.feat_layer,cam_label=cam_label, view_label=view_label)
        RGB_cash, RGB_global,RGB_map = self.RGB(RGB,layer=self.feat_layer, cam_label=cam_label, view_label=view_label)
        TPM_feature = self.TPM(RGB_cash[self.layer], NI_cash[self.layer], TI_cash[self.layer])
        ...
        if self.neck_feat == 'after':
            ...
            TPM_global = TPM_feature
        return torch.cat([TPM_global], dim=-1), RGB_global, NI_global, TI_global, RGB_map, NI_map, TI_map

__factory_T_type = {
    'vit_att': vit_base_att,
...
def make_model(cfg, num_class, camera_num, view_num=0):
    ...
    elif cfg.MODEL.BASE == 0:
        model = TOPReID_att(num_class, cfg, camera_num, view_num, __factory_T_type)
        print('===========Building TOPReID_att===========')
    else:
