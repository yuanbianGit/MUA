...
class Trans_att(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id, layer=1):
        B = x.shape[0]
        x = self.patch_embed(x)
        ...
        for index, blk in enumerate(self.blocks):
            ...
            if index == layer:
                ...
                inter_map = x
        cash_x.append(self.norm(x))
        return cash_x, inter_map

    def forward(self, x, cam_label=None, view_label=None, layer=1):
        x, inter_map = self.forward_features(x, cam_label, view_label,layer)
        return x,inter_map
...
def vit_base_att(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0,
                         drop_path_rate=0.1, camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = Trans_att(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, \
