...
class RGBNT201(BaseImageDataset):
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        for img_path_RGB in img_paths_RGB:
            ...
            img_path_NI_mask = osp.join(dir_path, 'NI_MASK', png_name)
            img_path_TI_mask = osp.join(dir_path, 'TI_MASK', png_name)
            img_path_RGB_mask = osp.join(dir_path, 'RGB_MASK', png_name)
            img_mask.append(img_path_RGB_mask)
            img_mask.append(img_path_NI_mask)
            img_mask.append(img_path_TI_mask)
            ...
            if relabel:
                pid = pid2label[pid]
            data.append((img,img_mask, pid, camid, trackid))
            # print("11111")
        return data
