...
class trans_mask:
    ...
    def __call__(self, input):
        # input = input.long()
        tensor_replaced = torch.where(input == 0.0, torch.tensor(-1.0), input)
...
#     """
#     imgs, imgs_mask, pids, camids, viewids, _ = zip(*batch)
...
#
#         RGB_mask_list.append(img_mask[0])
#         NI_mask_list.append(img_mask[1])
#         TI_mask_list.append(img_mask[2])
#
#     RGB = torch.stack(RGB_list, dim=0)
#     NI = torch.stack(NI_list, dim=0)
#     TI = torch.stack(TI_list, dim=0)
#
#     RGB_mask = torch.stack(RGB_mask_list, dim=0)
#     NI_mask = torch.stack(NI_mask_list, dim=0)
#     TI_mask = torch.stack(TI_mask_list, dim=0)
#
#     imgs = {'RGB': RGB, "NI": NI, "TI": TI}
#     imgs_mask = {'RGB': RGB_mask, "NI": NI_mask, "TI": TI_mask}
#     return imgs,imgs_mask, pids, camids, viewids,_
...
def make_dataloader(cfg):
    ...
    # train_set = ImageDataset(dataset.train, train_transforms,transforms_mask)
    # train_set_normal = ImageDataset(dataset.train, val_transforms,transforms_mask)
